...
#define OCTAVE 2
#define FAR 1000.0
#define MAX_STEPS 4 * 4 //max iteration on the marching loop
#define MAX_STEPS_SHADOW 4 * 2 //max iteration on the marching loop for shadow
...
vec2 getDist(vec3 p){
  ...
  float fbm = fbm(p + vec3(0, 0, -time * 100.0), 0.05, 0.005, 1.5, 4.0, 0.0, 0.0);
  // float dw = domainWarping(p + vec3(0, 0, -time * 100.0), 10, 10, 0.05, 0.005, 0.2, 1.0, 0.5, 0.25);
  // float test = smoothstep(0., .25, fbm);
  planeDist.x = planeDist.x - (fbm * 6000.);


  vec2 depth = planeDist;
  depth.x *= 0.35;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    //texturing
    vec2 uv = topDownUvProjection(pos, 0.005, vec2(500));
    ...
    //material;
    VoroStruct voronoi = voronoiDistance(uv, vec2(5.0), time.time, 0.25, 0.65);
    float thickness = 0.005;
    float smoothness = 0.015;
    float border = smoothstep(thickness, thickness + smoothness, voronoi.dist.x);

    vec3 mat = vec3(mix(0.25, 1.0, border));
    ...
    vec3 lightsColors[3] = vec3[3](
      vec3(1.0) * 0.25,//0.302, 0.3686, 1.0) * 0.25,
      vec3(1.0) * 1.0,//1.0, 0.9137, 0.6235) * 1.0,
      vec3(1.0) * 0.25//, 0.9529, 0.7922) * 0.15 //1.00,
      );
