...
#define OCTAVE 4
#define FAR 1000.0
#define MAX_STEPS 32 * 1 //max iteration on the marching loop
...
vec2 getDist(vec3 p){
  ...
  vec2 ground = sdSphere(p + vec3(0.0, 1000, 0), 1200, 0.0);

  VoroStruct voronoi = voronoiDistance(p.xz * GRIDRES, vec2(4), time, 0.1, 0.65);
  float rndIndex = random(voronoi.indices);
  float dist = smoothstep(0.0, 0.125, voronoi.dist.y);
  ground.x -=  rndIndex * dist * 10.0;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg =vec3(0.9176, 0.9686, 1.0);// vec3(0.1137, 0.1137, 0.1137) * 0.015;
  ...
  if(d.x > 0.0){
    ...
    vec3 mat = vec3(0.8196, 0.8784, 1.0);//vec3(0.0667, 0.0667, 0.0667);;// vec3(0.1098, 0.0941, 0.1686);
 
    
    VoroStruct voronoi = voronoiDistance(pos.xz * GRIDRES, vec2(4), time.time, 0.1, 0.65);
    float line = smoothstep(0.1, 0.125, voronoi.dist.y) -smoothstep(0.125, 0.15, voronoi.dist.y) ;

    
    mat += vec3(line * 0.25) + nor.zxy * 0.05;
    // mat =  vec3(fpos.x, fpos.y, fpos.z) + isSquare;

    float specMat = random(voronoi.indices);
    // mat = vec3(specMat);
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      float diffM =  clamp(dot(nor, lp), 0.0, 1.0) 
                    * softShadowImproved(pos, lp, 100, 1000.0 );
