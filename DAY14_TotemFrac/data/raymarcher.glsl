...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    lightColor *= 0.65;
    float eta   = PI * 0.1;//0.22;
    float theta = TWO_PI * 0.22;
	...
	// diff += 0.5;
	   vec3 shadow = mix((vec3(166, 155, 151) / 255.0), mat, diff * 0.35);
    //material definition
    col += shadow * lightColor + rim * rimPower;
    col += shadow * speItensity * specular * lightColor;
