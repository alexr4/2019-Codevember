class DGS {
  ...
  void initAsCircle() {
    ...
    for (int i=0; i<nbElement; i++) {
      float normIndex = (float)i/ (float)(nbElement-1);
      float theta = normIndex * TWO_PI;

      float x = cos(theta) * radius + width/4;
      float y = sin(theta) * radius + height/4;
  ...
  void initAsSpiral() {
    ...
    for (int i=0; i<nbElement; i++) {
      float normIndex = (float)i/ (float)(nbElement-1);
      float theta = normIndex * TWO_PI;
      float rad = radius * normIndex;
      float x = cos(theta) * rad + width/2;
      float y = sin(theta) * rad + height/2;
      PVector location = new PVector(x, y);
      Node node = new Node(location);
      nodeList.add(node);
    }
  ...
  void computeDebugBuffer(PGraphics buffer, float sw, float hue, ArrayList<Node> nodeList) {
    ...
    for (int i=0; i<nodeList.size(); i++) {
      Node node = nodeList.get(i);
      float normi = (float)i/(float)nodeList.size();
      buffer.fill(normi, 1.0, 1.0);
