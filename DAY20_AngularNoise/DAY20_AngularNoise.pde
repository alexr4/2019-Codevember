...
void setup() {
  ...
  filterpp        = new Filter(this, CONFIG.originalWidth, CONFIG.originalHeight);
  ...
  layers.add(filterpp.getBuffer());

  lut = loadImage("lut1d.png");
...
void draw() {
  ...
  raymarcher.set("time", Time.time);

  compute(livecoding);
  computeBuffer(ctx);
  computeUIBuffer(ui);
  computePostProcessBuffer(ctx);
  computeOutputBuffer(outputBuffer, new PGraphics[]{ui, filterpp.getBuffer()});
  ...
  if (debug) {
    ...
    image(ppb.getSrcBuffer(), 0, 60);
    image(ppb.getDstBuffer(), ppb.getDstBuffer().width + 20, 60);
    pt.display(0, 0);
  } else {
...
void computeUIBuffer(PGraphics ui) {
  ui.beginDraw();
  ui.background(0.902 * 255, 0.8706 * 255, 0.8471 * 255);
...
void computePostProcessBuffer(PGraphics src) {
  filterpp.getLut1D(src, lut);
  //1- High pass the source image
  filterpp.getHighPass(filterpp.getBuffer(), 2.0);
  //2- Desaturate the result image
  filterpp.getDesaturate(filterpp.getBuffer(), 100.0);
  //3- Compose it with the source image as overlay
  comp.getBlendOverlay(filterpp.getBuffer(), src, 100.0);

  filterpp.getChromaWarpHigh(comp.getBuffer(), src.width/2, src.height/2, 0.001, HALF_PI * 0.005);
  filterpp.getAnimatedGrainRGB(filterpp.getBuffer(), 0.01);
}
...
void keyPressed() {
  ...
  case 'S':
    ...
    filterpp.getBuffer().save(CONFIG.exportPathImage+filename);
