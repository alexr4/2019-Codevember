...
vec2 topDownUvProjection(vec3 pos, float textureFreq, vec2 offset){
    vec2 uv = textureFreq * (pos.xz + offset);
    uv.x = 1.0 - uv.x;
...
vec2 getDist(vec3 p){
  ...
  float dist = length(p.xz) / 100.0;
  vec3 np = vec3(sin(angle + PI) * 0.5 + 0.5 + time, 
                cos(angle + PI) * 0.5 + 0.5 + time, 
                    time);

  vec2 uv = topDownUvProjection(p, 0.005, vec2(100));
  float displacement = texture2D(displacementMap, uv).r;
  vec3 newposition = vec3(dist) * 5.0 - time * 0.001 + (sin(angle * 8.0) * 0.5 + 0.5) * dist + (sin(p * 0.1 + time * 0.001) * 0.5 + 0.5) * 0.5;
  
  float elevation = fbm(newposition, dist, 1.0, 1.0, 1.0, 0.0, 0.0);
  
  vec2 cyd = sdCylinder(p, vec3(0, 150, 0), vec3(0, -25, 0), 100, .0);
  // vec2 box = sdBox(p, vec3(100, 25, 100), 0.0);
  cyd.x -= elevation * 30.0 * (1.0-dist);
  cyd.x -= 2.0;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = vec3(0.902, 0.8706, 0.8471);;
  bg *= 0.85;
  ...
  if(d.x > 0.0){
    ...
    vec3 lightsPos[3] = vec3[3](
    	vec3(cos(PI * 0.9), -0.15, sin(PI * 0.9)),
    	vec3(cos(PI * 0.05), -0.25, sin(PI * 0.05)),
    	vec3(0.5, -1, 0)
    	);
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.0235, 0.1686, 1.0) * 1.0,//0.25,
    	vec3(1.0, 0.9137, 0.6235) * 1.0,//1.0,
    	vec3(1.0, 0.9529, 0.7922) * 0.25//0.15 //1.00,
    	);
    ...
    //fog of war
    float fog = (d.x - near) / (far*0.5 - near);
    fog = clamp(fog, 0., 1.0);
...
void main(){
  //define time
  Time stime = computeTime(time, 15000.0);
  ...
  vec3 ro, rd;
  //x
  ro =  vec3(cos(time * 0.00005 * TWOPI) * FAR * 0.1,
  			      -50 - inoutquad(abs(stime.normTime * 2.0 - 1.0)) * 200,
  			      sin(time * 0.00005* TWOPI) * FAR * 0.1);
  // ro =  vec3(0,
  ...
  // ro = vec3(0, -250 * mouse.y, -FAR * 0.25);
  ...
  // gamma
  color.rgb = toGamma(color.rgb * 1.1);
