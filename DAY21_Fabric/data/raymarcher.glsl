...
vec2 getDist(vec3 p){
  vec2 uv = topDownUvProjection(p, 0.025, vec2(100));
  uv = fract(uv);
  ...
  vec3 rpx = rotation(p, vec3(1, 0, 0), PI * 0.5  + time * PI);
  vec3 rpy = rotation(p, vec3(0, 1, 0), PI * 0.75 + time * PI);
  vec3 rpz = rotation(p, vec3(0, 0, 1), PI * 0.25 + time * PI);
  float incRot = 0.75;
  vec3 sp = vec3(sin(p.x * 0.5 + time * incRot * PI)    * cos(rpz.z * 0.1257 + time * incRot * PI)    * sin(rpy.y * 0.05 + time * incRot * PI),
                 sin(p.y * 0.5 - time * incRot * TWOPI) * sin(rpz.z * 0.1257 - time * incRot * TWOPI) * sin(rpx.x * 0.05 - time * incRot * TWOPI),
                 cos(p.z * 0.5 - time * incRot * PI) * sin(rpy.y * 0.1257 + time * incRot * TWOPI) * cos(rpx.x * 0.05 - time * incRot * PI));
    
  float noised0 = snoise(sp* 0.25 + vec3(0.0, 0.0, time));
  ...
  float sSize = abs(sin(time * 0.5) * 50 + 200); 
  p.z = p.z + sin(ny * TWOPI + time * 1.5) * 10.5;
  // p.x = p.x + sin(ny * TWOPI + time * 0.75) * 5.0;
  vec3 pos = opRepeat(p, vec3(10, 0.001, 0.0));
  vec2 sphere = sdSphere(pos + vec3(-95, 95, 0), sSize, 0.0);
  sphere.x -= noised0 * 1.0 * sSize / 250.0;//FBM

  vec2 scene = sphere;
  scene.x -= displacement * 0.25;
  scene.x *= 0.05;
  return scene;
}
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    //texturing
    vec2 uv = topDownUvProjection(pos, 0.5, vec2(0));
    uv = fract(uv);
    ...
    nor = normalMapping ;
    uv = topDownUvProjection(pos, 0.01, vec2(50));
    // vec3 albedoMapping    = texture2D(albedoMap, uv).xyz;
    
    //material;
    vec3 albedoMapping = triplanarMap(pos, nor, albedoMap);
    vec3 mat = albedoMapping * 0.75;//vec3(0.5);
    ...
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.302, 0.3686, 1.0) * 1.0,//0.25,
    	vec3(1.0, 0.9137, 0.6235) * 1.0,//1.0,
    	vec3(1.0, 0.9529, 0.7922) * 1.15//0.15 //1.00,
    	);
    
    vec3 SScol = albedoMapping;
    float density = 0.1 * 10.0;
    float intensity = 2.0;
    ...
    vec3 perturb = vec3(
      sin(snoise(pos.zyx * .01 + nor.zyx * 0.01 + time.time * 0.1)),
      sin(snoise(pos.yxz * .01 + nor.yxz * 0.01 + time.time * 0.1)),
      sin(snoise(pos.xzy * .01 + nor.xzy * 0.01 + time.time * 0.1))
      );

    // vec3 perturb = pos;
    vec3 specIri = spectrum(dot(nor + perturb * 0.5, eye) * 3.5 + time.time * 0.5) * pow(specularMapping.r, mouse.x * 10.0);
    ...
    //material definition
    // col += (SScol * subsurface) * intensity + mat * lightColor + specColor + (SScol * subsurface) * intensity + pow(specIri, vec3(mouse.x)) * lightColor * perturb.x  * specularMapping.r ;//Lambert estimation + BlinnPhong
    col += mat * lightColor + specColor + pow(specIri, vec3(1.25)) * lightColor * perturb.x * specularMapping.r * 0.5;//Lambert estimation + BlinnPhong
    ...
    ndofLength       = clamp(ndofLength,  0.005, 21.0);
