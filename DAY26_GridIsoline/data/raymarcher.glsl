...
vec2 getDist(vec3 p){
  ...
  vec2 uv = p.yz * vec2(n.x) +
            p.xz * vec2(n.y) +
            p.xy * vec2(n.z);
  VoroStruct voronoi = voronoiDistance(uv * 0.001, vec2(10.0), time * 0.5, 0.5, 0.75);

  vec2 ground = sdPlane(p + vec3(0, 260, 0), vec4(0, 1, 0, 1), 0.0);

  // vec2 sphereOut = sdSphere(p, 250, 20.0 + (voronoi.indices.x + voronoi.indices.y * 10.0));
  vec2 sphereIn = sdSphere(p, 400 * 0.61, 1.0);
  // sphereOut.x += voronoi.dist.y * 50;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = vec3(0.0745, 0.0353, 0.2471) * 0.015;
  ...
  if(d.x > 0.0){
    ...
    vec3 mat =vec3(0.2314, 0.0706, 0.3255);;// vec3(0.1098, 0.0941, 0.1686);
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      float specPower = (100.0 * ground + 500 * specMat) * (1.0 - isSquare) + 10.0 * isSquare;
      float gloss = (100.0 * ground  + 500 * specMat) * (1.0 - isSquare) + 10.0 * isSquare;
