...
vec2 getDist(vec3 p){
  ...
  vec3 n = normalize(p);
  vec2 uv = p.yz * vec2(n.x) +
            p.xz * vec2(n.y) +
            p.xy * vec2(n.z);
 
 float ny = (p.y + 260) / (260 * 2.0);
 vec3 np = rotation(p, vec3(1.0), ny * PI);
  np = rotation(np, vec3(1.0), PI + time * 0.5);
 float noise = snoise(p * 0.0025 + uv.xyx * 0.0025);
 float noisey = abs(sin(snoise(vec3(p.y * 0.05)) * 0.5 + 0.5 * TWOPI));

  vec2 ground = sdPlane(p + vec3(0, 260, 0), vec4(0, 1, 0, 1), 0.0);

  vec2 sphereIn = sdCylinder(np, vec3(0, 260, 0), vec3(0, -260, 0), 160, 1.0);
  sphereIn.x -= noise * 50 + noisey * 4.5;
  vec2 scene = sphereIn;//opUnite(sphereOut, sphereIn);
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = vec3(0.1137, 0.1137, 0.1137) * 0.015;
  ...
  if(d.x > 0.0){
    ...
    vec3 mat =vec3(0.0667, 0.0667, 0.0667);;// vec3(0.1098, 0.0941, 0.1686);
    ...
    float noisedSSS = snoise(sin(vec3(pos.y) * 0.1)) * 0.5 + 0.5;
    vec3 sss0 = vec3(1.0, 0.6314, 0.0824);
    vec3 sss1 = vec3(1.0, 0.3412, 0.0824);
    vec3 SScol = mix(sss0, sss1, noisedSSS);
    float density = 10;
    float intensity = 20.75;
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      ss = ss * 0.5 + pow((d.x / FAR) * 2.0, density);
      subsurface += ss;
    }
    ...
    //material definition
    col += (SScol * subsurface) * intensity * (1.0 - ground) + mat * lightColor + specColor + (SScol * subsurface) * intensity * (1.0 - ground) ;//Lambert estimation + BlinnPhong
