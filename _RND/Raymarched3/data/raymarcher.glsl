...
vec2 getDist(vec3 p){
  ...
  float noiseShapeScale = random(floor(time + 10.0)) * 0.25 + 0.25;
  float noisedShape = snoise(vec3(vec2(npy * 4.0), floor(time))) * 0.5 + 0.5;
  npy = p.y + noisedShape * noiseShapeScale * maxScale;
  p.y = npy;

  //shapes 
  float rndSizeSphere       = (random(vec2(floor(time + 20.0))) * 0.25 + 0.75) * boxRadius * 0.85;
  float rndPosSphere        = (random(vec2(floor(time + 70.0))) * 2.0 - 1.0) * boxRadius * 0.25;
  float rndSizeTop          = (random(vec2(floor(time + 30.0))) * 0.5 + 0.5) * boxRadius * 0.35;
  float rndSizeBaseTop      = (random(vec2(floor(time + 40.0))) * 0.75 + 0.25) * boxRadius * 0.25;
  float rndSizeBaseBottom   = (random(vec2(floor(time + 50.0))) * 0.25 + 0.25) * boxRadius * .5;
  float rndSmooth           = (random(vec2(floor(time + 60.0))) * 0.75);
