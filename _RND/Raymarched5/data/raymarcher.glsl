...
vec2 getDist(vec3 p){
  ...
  vec2 sphere = sdSphere(p, 50, 0.0);
  sphere.yx += sin(uv.y * 100.0 * PI) * 100 * mouse.x;
  vec3 op = p;
  ...
  for(int r=0; r<ROWS; r++){
  	for(int c=0; c<COLS; c++){
  		float nr = float(r)/float(ROWS);
  		float nc = float(c)/float(COLS);

  		float eta = nr * PI;
  		float theta = nc * TWOPI + mod(float(r), 2.0) * PI * 0.1;

  		float offset = 100 * 0.58;
  		vec3 ps = vec3(
  			sin(eta) * cos(theta) * offset,
  			sin(eta) * sin(theta) * offset,
  			cos(eta) * offset) + op;

  		vec2 spheres = sdSphere(ps, 10, 0.0);
  		sphere = opSubstract(spheres, sphere);
  	}
  }
