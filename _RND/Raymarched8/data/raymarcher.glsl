...
vec2 getDist(vec3 p){
  ...
  float roundCorner = 4.0;
  float AMult = 10.0;
  float AMinSize = 50;
  float AMaxSize = 200 - AMinSize;
  vec3 rndAAxis = random3D(vec3(stime.timeLoop * AMult));//random(float(stime.timeLoop));
  float rndAAngle = random(float(stime.timeLoop * AMult) + 100.0);
  vec3 rndASize = (random3D(vec3(stime.timeLoop * AMult) + 200) * 0.5 + .5) * AMaxSize + vec3(AMinSize);
  vec3 posA = rotation(p, rndAAxis, TWOPI * rndAAngle);

  float BMult = 20.0;
  float BMinSize = 50;
  float BMaxSize = 200 - BMinSize;
  vec3 rndBAxis = random3D(vec3(stime.timeLoop * BMult));//random(float(stime.timeLoop));
  float rndBAngle = random(float(stime.timeLoop * BMult) + 100.0);
  vec3 rndBSize = (random3D(vec3(stime.timeLoop * BMult) + 200) * 0.5 + .5) * BMaxSize + vec3(BMinSize);
  vec3 posB = rotation(p, rndBAxis, TWOPI * rndBAngle);

  vec2 box = sdBox(posA, rndASize, 0.0) - roundCorner;
  vec2 sphere = sdSphere(posA, rndASize.x, 0.0) - roundCorner;

 

  vec2 scene = opSmoothUnite(box, sphere, 25.0);
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = vec3(0.0902, 0.3843, 0.7725);;
  bg *= 0.01;
