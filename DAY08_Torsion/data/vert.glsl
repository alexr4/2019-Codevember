...
void main() {
  FragOut.normal = normalize(normalMatrix * normal);
   float df = snoise(position.xyz * noiseInc + time);
   df = abs(df);
   vec4 displacedVector = position +  vec4(normal * displacementFactor * df * 1.0, 0.0) * easing;
  
  gl_Position = transformMatrix * displacedVector;
  FragOut.vertex = (transformMatrix * displacedVector).rgb;
  vec3 ecp = vec3(modelviewMatrix * displacedVector);
  FragOut.ecVertex = ecp;
  FragOut.color =  vec4(213, 226, 242, 255) / 255.0;
