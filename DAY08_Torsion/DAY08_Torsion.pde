...
void setup() {
  ...
  head = loadShape(MEDIA.path+"_Models/einstein_SimplifyF.obj");
  //head.enableStyle();
...
void draw() {
  ...
  if (previousLoop != ctime.timeLoop && ctime.timeLoop % 3 == 0) {
    ...
    noiseInc = random(0.1, 0.25);
    displacementFactor = random(0.9, 1.25);
    println("reset");
  } else {
  }
...
void computeBuffer(PGraphics ctx) {
  ...
  //lighting
  SimplePBR.setDiffuseAttenuation(mat.getShader(), 2.5);
  SimplePBR.setReflectionAttenuation(mat.getShader(), 0.01);
  SimplePBR.setExposure(mat.getShader(), 1.35);
  ...
  mat.setFloat("displacementFactor", displacementFactor);
  mat.setFloat("noiseInc", noiseInc);
  mat.setFloat("time", Time.time * 0.0001);
  mat.setFloat("easing", easedTime);

  mat.setRougness(1.0);
  mat.setMetallic(0.00);
  mat.setRim(0.0);
