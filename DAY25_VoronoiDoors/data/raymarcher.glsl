...
#define MAX_STEPS_SHADOW 32 * 1 //max iteration on the marching loop for shadow
...
vec2 getDist(vec3 p){
  ...
  vec3 rp = opRepLim(p, 260, vec3(1, 0, 0));
  vec3 inc = round(p/260);
  //p = rotation(p, vec3(1, 0, 0), time* 0.1);
  vec2 uv = topDownUvProjection(rp, 0.0018, vec2(280, 270));
  uv = fract(uv + vec2(0, time * 0.0));
  uv.xy = rotation(vec3(uv, 0.0), vec3(0., 0, 1.0), time * 0.1).xy;
  // float displacement = texture2D(displacementMap, uv).r;
  float noised = snoise(p * 0.001 + time * 0.1);
  VoroStruct voronoi = voronoiDistance(uv + inc.x, vec2(10.0), time * 0.5, 0.5, 0.75);
  float displacement = voronoi.dist.y;

  float ny = (rp.y/250.0);// - 250 * 0.5) / 250.0;
  vec2 planeRD = sdBox(rp, vec3(100, 250, 2), 0.0);
  float stepper = step(0.025 + 0.15 * (1.0 - ny), displacement);
  planeRD.x += stepper * 3.0;// * easing;

  vec2 plane = sdBox(rp, vec3(100, 250, 2), 0.0);;

  
  vec2 border = sdBox(rp, vec3(110, 260, 4), 0.0);
  vec2 borderExclude = sdBox(rp, vec3(100, 250, 10), 0.0);
  border = opSubstract(borderExclude, border);

  vec2 ground = sdPlane(p + vec3(0, 260, 0), vec4(0, 1, 0, 1), 1.0);

  vec2 scene = planeRD;
  scene = opUnite(border, scene);
  scene = opUnite(ground, scene);
  // scene.x *= 0.75;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    float ground = step(0.5, index);
    vec3 mat =vec3(0.8627, 0.8314, 1.0) * 0.75 * (1.0 - ground) + vec3(0.1098, 0.0941, 0.1686) * ground; 

    vec3 posGround = fract(pos * 0.025);
    float thickness = 0.025;
    float isSquare = step(posGround.x, thickness) + 1.0 - step(posGround.x, 1.0 - thickness) + 
                    step(posGround.y, thickness) + 1.0 - step(posGround.y, 1.0 - thickness) + 
                    step(posGround.z, thickness) + 1.0 - step(posGround.z, 1.0 - thickness) ;
    isSquare = clamp(isSquare, 0.0, 1.0)                ;
    // mat += vec3(isSquare);

    float vscale = 0.0015;
    VoroStruct voronoixy = voronoiDistance(pos.xy * vscale, vec2(10), time.time, 0.25, 1.0);
    VoroStruct voronoixz = voronoiDistance(pos.xz * vscale, vec2(10), time.time, 0.25, 1.0);
    float rndSpec = random(voronoixy.indices) * (1.0 - ground) + random(voronoixz.indices) * ground;
    float vbase = 0.5;
    float vthickness = 0.1;
    float vsmoothness = 0.15;
    float linexz = smoothstep(vbase - vthickness * 0.5 - vsmoothness, vbase - vthickness * 0.5, fract(voronoixz.dist.y * 6.0)) 
                  - smoothstep(vbase + vthickness * 0.5, vbase + vthickness * 0.5 + vsmoothness, fract(voronoixz.dist.y * 6.0));
    float linexy = smoothstep(vbase - vthickness * 0.5 - vsmoothness, vbase - vthickness * 0.5, fract(voronoixy.dist.y * 6.0))
                - smoothstep(vbase + vthickness * 0.5, vbase + vthickness * 0.5 + vsmoothness, fract(voronoixy.dist.y * 6.0));

    vec3 specmat = vec3(linexz * ground + linexy * (1.0 - ground));
    mat += specmat * 0.05;
    ...
    vec3 lightsPos[3] = vec3[3](
    	...
    	vec3(cos(TWOPI * .525), 1.0, sin(TWOPI * .525)),
    	vec3(0.15, 0.5, -0.5)
    	);
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.2588, 0.4314, 0.9882) * 0.5,//0.25,
    	vec3(1.0, 1.0, 1.0) * 0.5,
    	vec3(1.0, 0.9529, 0.7922) * 1.0 //1.00,
    	);
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      float specPower = 10.0 * ground + 100 * (1.0 -ground) + 500 * (rndSpec + specmat.x);
      float gloss = 10.0 * ground + 100 * (1.0 -ground) + 500 * (rndSpec + specmat.x);
...
void main(){
  //define time
  Time stime = computeTime(time, 10.0);
  ...
  ro =  vec3(
  			cos(stime.normTime * TWOPI) * FAR * 0.65,
  			abs(sin(stime.normTime * TWOPI) * 300.0) + 0.0,
  ...
  // gamma
  color.rgb = toGamma(color.rgb * 1.5);
