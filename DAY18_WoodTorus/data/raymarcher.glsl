...
vec2 getDist(vec3 p){
  ...
  p = rotation(p, vec3(1, 0, 0), PI * 0.5);
 

  //torus
  vec2 torus = sdTorus(p, vec2(150, 40), 0.0);
  // float torusNoised = snoise(p * 0.025 + time);
  float torusNoised = fbm(p * 0.015 + vec3(0, 0, time * 1.0), 0.35, 1.75, 1.0, 1.0) * 0.5 + 0.5;
  vec2 toCenter = -p.xz;
  ...
  float angleStepper = smoothstep(angleStep - thickness - smoothness, angleStep - thickness, fract(angle / TWOPI + time * timeScale)) * //easing
                      (1.0 - smoothstep(angleStep + thickness, angleStep + thickness + smoothness, fract(angle / TWOPI + time * timeScale)));
  torus.x -= torusNoised * 35 * angleStepper;
  torus.x -= displacement * 1.0;
  torus.x *= 0.5;
  return torus;
}
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = (vec3(18, 10, 5) * 1.5) / 255.0; //40, 37, 35
  bg *= 0.095;
  ...
  if(d.x > 0.0){
    ...
    vec3 normalMapping    = mix(nor, perturb_normal(nor, normalize(view), uv, normalMap), 0.5);
    ...
    vec3 lightsPos[4] = vec3[4](
    	vec3(cos(TWOPI * 0.5), -1.0, sin(TWOPI * 0.48)),
    	vec3(cos(PI * 0.05), -0.8, sin(PI * 0.05)),
    	vec3(0.25, -0.75, -0.015),//cos(TWOPI * 0.9) * 1.0, -1, sin(TWOPI * 0.9) * 1.0
    	vec3(cos(-PI * 0.7), 0, sin(-PI * 0.7))
    	);
    vec3 lightsColors[4] = vec3[4](
    	vec3(0.0235, 0.1686, 1.0) * 0.55,
    	vec3(1.0, 0.9137, 0.6235) * 1.0,//1.0
    	vec3(1.0, 0.9137, 0.6235) * 0.25,
    	vec3(0.9373, 0.9804, 0.9882) * 0.1 //0.35
    	);
...
void main(){
  ...
  vec3 ro, rd;
  //x
  ro =  vec3(cos(stime.normTime * TWOPI) * FAR * 0.5,
  			sin(time * 0.15) * 0.25 * PI * 400,
  			sin(stime.normTime * TWOPI) * FAR * 0.5);

  // ro = vec3(sin(time * 0.15) * 0.25 * FAR, 
  //           sin(time * 0.15) * 0.25 * PI * 200, 
  //           FAR * 0.45);// (mouse.x * 0.5 + 0.5));
  // ro = vec3(0, 0, FAR * 0.25);
  // ro =  vec3(cos((mouse.x +0.25)* TWOPI) * FAR * 0.5,
  // 			0,
  // 			sin((mouse.x + 0.25) * TWOPI) * FAR * 0.5);

  float hyp = sqrt(resolution.x * resolution.x + resolution.y * resolution.y) * 0.5;
  rd = R(uv, ro, vec3(0.0), vec3(0.0, -1, 0.25), PI * 0.5);
