...
//NB : this version use an MRAOC map where : 
/*
...
void main() {
    ...
    */
    ...
    float roughness = texture2D(mraocMap, texcoord).y * material.y;
 #else
    float roughness = material.y;
 #endif

 #ifdef USE_METALNESS_MAP
     float metallic = texture2D(mraocMap, texcoord).x * material.x;
 #else
