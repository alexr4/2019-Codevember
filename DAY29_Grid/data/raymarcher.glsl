...
vec2 getDist(vec3 p){
  ...
  vec3 posGround = p * GRIDRES;  
  vec3 fpos = fract(posGround);
  vec3 index = floor(posGround);
  float rndElevation = random(index.xz);

  float elevation = 25;
  float siny = sin(p.z * 0.0025 + time * 0.5) * elevation * 0.35;

  float dist = length(vec2(0.5) - fpos.xz);
  dist = smoothstep(0.025, 0.5, dist);
  float spikeElevation = step(0.5, rndElevation) * (1.0 - dist);
  ground.x -=  (spikeElevation * elevation) * (sin(index.x + index.y + time));
  ground.x -= elevation + sin(p.x * 0.0075 + p.z * 0.0075 + time) * elevation + 
              elevation + cos(siny + time) * elevation;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    vec3 mat = vec3(0.05);//vec3(0.0667, 0.0667, 0.0667);;// vec3(0.1098, 0.0941, 0.1686);
 
    vec3 posGround = pos * GRIDRES;
   
    vec3 fpos = fract(posGround);
    vec3 index = floor(posGround);
    float thickness = GRIDRES;
    ...
    mat += vec3(isSquare * 0.05);
    // mat =  vec3(fpos.x, fpos.y, fpos.z) + isSquare;
    ...
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.0588, 0.2784, 1.0) * 1.0,//0.25,
    	vec3(0.8, 0.902, 0.9882) * 1.0,
    	vec3(0.298, 0.0, 1.0) * 0.5 //1.00,
    	);
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      float diffM =  clamp(dot(nor, lp), 0.0, 1.0) 
                    * softShadowImproved(pos, lp, 20, 100.0 );
      ...
      float specPower = 10.0 + 500 * specMat;
      float gloss = 10.0 + 500 * specMat;
...
void main(){
  ...
  ro =  vec3(
  			...
  			abs(sin(stime.normTime * TWOPI) * 300.0) + 100,
  ...
  rd = R(uv, ro, vec3(0.0, 0, 0), vec3(-0.25, 1, 0.0), PI * 0.5);
