...
vec2 getDist(vec3 p){
  ...
  vec2 ground = sdPlane(p + vec3(0, -10, 0), vec4(0, 1, 0, 1), 0.0);

  float npy = (p.y + 1e3) / (1e3 * 2.0);
  p = rotation(p, vec3(0, 1, 0), p.y *0.00025 * TWOPI);
  p = opRepLim(p, 15, vec3(0, 0, 2));
  p = opRepLim(p, 50, vec3(4, 0, 0));

  float smoothness = 0.05;
  float thickness = 0.01;
  float edge = easing;
  float repeat = 1.0 - npy;
  float ssnpy = smoothstep(edge - thickness - smoothness, edge - thickness, repeat) - 
                smoothstep(edge + thickness, edge + thickness + smoothness, repeat);
  vec2 cyl = sdCylinder(p, vec3(0, 0, 0), vec3(0, 1e10, 0), 5 + 10 * ssnpy, 1.0);
  cyl.y = mix(10.0, 100.0, ssnpy);

  vec2 scene = cyl;//opUnite(sphereOut, sphereIn);
  // scene = opUnite(cyl, scene);
  // scene.x *= 0.75;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  vec3 bg = vec3(0.0431, 0.0, 0.4196);// vec3(0.1137, 0.1137, 0.1137) * 0.015;
  ...
  if(d.x > 0.0){
    ...
    vec3 pos = ro + rd * d.x;
    vec3 nor = getNormal(pos, 0.001);
    vec3 eye = normalize(ro);
    vec3 view = normalize(-rd);
    
    float normalIndex = (index - 10.0) / (100.0-10.0);
    float light = normalIndex;
    float isLight = (index >= 10.0) ? light : 0.0;
    ...
    float ground = (index == 0) ? 1.0 : 0.0;
    vec3 mat = (vec3(0.05) + pos * 0.00015) * ground 
    + (vec3(0.1294, 0.1647, 0.2392)) * ((1.0 - ground) * (1.0 - isLight));//vec3(0.0667, 0.0667, 0.0667);;// vec3(0.1098, 0.0941, 0.1686);
 
    vec3 posGround = pos * 0.025 + time.time * (1.0 - ground);
    vec3 posTube = pos * 0.075 + time.time * (1.0 - ground);
    vec3 fposTube = fract(posTube);
    vec3 fpos = fract(posGround);
    vec3 index = floor(posGround);
    float thickness = 0.025;
    float isSquare = step(fpos.x, thickness) + 1.0 - step(fpos.x, 1.0 - thickness) + 
                    step(fpos.y, thickness) + 1.0 - step(fpos.y, 1.0 - thickness) + 
                    step(fpos.z, thickness) + 1.0 - step(fpos.z, 1.0 - thickness) ;
    isSquare = clamp(isSquare, 0.0, 1.0);
    float lineSquared =step(fposTube.y, thickness * 2) + 1.0 - step(fposTube.y, 1.0 - thickness * 2);
    mat += vec3(isSquare) * ground + lineSquared * (1.0 - ground);
    // mat = mix(mat, vec3(1.0), lineSquared);

    float specMat = random(index);
    // mat = vec3(specMat);
    ...
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.2235, 0.4039, 1.0) * 1.0,//0.25,
    	vec3(1.0) * 1.0,
    	vec3(1.0, 0.9529, 0.7922) * 0.5 //1.00,
    	);
    
    float noisedSSS = snoise(sin(vec3(pos) * 0.015)) * 0.5 + 0.5;
    vec3 sss0 = vec3(0.4784, 0.0824, 1.0);
    vec3 sss1 = vec3(0.0, 0.4, 1.0);
    vec3 SScol = mix(sss0, sss1, noisedSSS);
    float density = 100;
    float intensity = 10.75;
    ...
    for(int i=0; i<lightsPos.length; i++){
      ...
      float specPower = (100.0  + 500 * specMat)* ground + 5 * (1.0 - ground);
      float gloss = (100.0 + 500 * specMat) * ground  +    5 * (1.0 - ground);
    ...
    rim = smoothstep(0.5, 1.0, rim) * (1.0 - ground);
	
    //material definition
    col += (SScol * subsurface) * intensity * isLight + 
           mat * lightColor + specColor + 
           (SScol * subsurface) * intensity * isLight;//Lambert estimation + BlinnPhong
...
void main(){
  ...
  ro =  vec3(
  			...
  			abs(sin(stime.normTime * TWOPI) * 300.0),
  ...
  rd = R(uv, ro, vec3(0.0, 300, 0), vec3(-0.25, 1, 0.0), PI * 0.5);
