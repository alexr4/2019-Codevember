...
void computeBuffer(PGraphics ctx) {
  ...
  SimplePBR.setReflectionAttenuation(mat.getShader(), 0.05);
  SimplePBR.setExposure(mat.getShader(), 1.5);
  ...
  mat.setFloat("iridescentRatio", 0.01);

  mat.setRougness(0.5);
  mat.setMetallic(0.25);
  mat.setRim(1.0);
  ...
  int rows = 30;
  float radiusXP = 250;
  float timeInc = Time.time * 0.0001;
  float noiseInc = 0.25;
  ...
  for (int i = 0; i<cols; i++)
  {
    ...
    { 
      ...
      float alpha = map(i, 0, cols-1, PI * 0.1, PI * 0.9);// + sin(Time.time * 0.0001);
      float beta = map(j, 0, rows-1, 0, TWO_PI);
      
      float index = i + j * cols;
      index /= (float)(cols * rows);
      float noise = noise(sin(alpha) * noiseInc + noiseInc +  timeInc + sin(beta) * noiseInc + noiseInc +  timeInc, 
                          cos(alpha) * noiseInc + noiseInc +  timeInc + cos(beta) * noiseInc + noiseInc +  timeInc, 
                          timeInc);
      //noise = 1.0;
      float x_ =  sin(alpha) * cos(beta) * (noise * radiusXP);
      float y_ =  sin(alpha) * sin(beta) * (noise * radiusXP);
      float z_ =  cos(alpha) * (noise * radiusXP);
      
      ctx.pushMatrix();
      ctx.translate(x_, y_, z_);
      ctx.scale(0.5 + 2.0 * noise);
...
void computeUIBuffer(PGraphics ui) {
  ui.beginDraw();
  ui.background(23 * 1.25, 12 * 1.25, 42 * 1.25);
