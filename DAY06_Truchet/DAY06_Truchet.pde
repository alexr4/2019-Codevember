...
void computeBuffer(PGraphics ctx) {
  ...
  int depth = round(gdepth/res);

  float noiseScale = 0.25;
  float noiseTimeScale = 0.0001;
  float noiseTimeScale2 = 0.001;
  float offset = 100;
  ...
  for (int d=0; d<depth; d++) {
    for (int r=0; r<rows; r++) {
      ...
      for (int c =0; c<cols; c++) {
        ...
        float pz = res * d + res * 0.5 - gdepth * 0.5;

        float noiseVal = noise(r * noiseScale - ctime.time * noiseTimeScale2, 
          c * noiseScale + ctime.time * noiseTimeScale2, 
          d * noiseScale + ctime.time * noiseTimeScale);
         float noiseValZ = noise(offset + d * noiseScale - ctime.time * noiseTimeScale2, 
          offset + r * noiseScale + ctime.time * noiseTimeScale2, 
          offset + c * noiseScale + ctime.time * noiseTimeScale);
        float noiseAngle = 0.0;
        float noiseAngleZ = 0.0;
        switch(round(noiseVal * 2)) {
        case 0 : 
          noiseAngle = HALF_PI;
          break;
        case 1 : 
        ...
        float nr = noise(r * noiseScale - ctime.time * noiseTimeScale2);
        float nc = noise(c * noiseScale + ctime.time * noiseTimeScale2);
        float nd = noise(d * noiseScale + ctime.time * noiseTimeScale2);

        ctx.pushMatrix();
        ctx.translate(px, py, pz);
        ctx.rotateZ(noiseAngle);
        //ctx.rotateY(noiseAngleZ);
        ctx.stroke(nr * 255, nc *255, nd * 255);
        
        //ctx.line(-res*0.5, 0.0, res*0.5, 0.0);
        ctx.arc(res*0.5, -res*0.5, res, res, HALF_PI, PI);
        ctx.arc(-res*0.5, res*0.5, res, res, HALF_PI + PI, TWO_PI);
