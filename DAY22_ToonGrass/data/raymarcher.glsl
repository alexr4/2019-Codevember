...
vec2 getDist(vec3 p){
  ...
  vec3 c = vec3(100, 0, 100);
  vec3 l = vec3(10.0);
  vec3 inc = round(p/c);
  vec3 q = p-c*clamp(round(p/c),-l,l);
  float eta = atan(q.z, q.x) + PI;
  q.z = q.z + sin(p.y * 0.01 + time * 2.0 + inc.x) * 10.0;
  q.x = q.x + sin(p.y * 0.01 + time * 2.0 + inc.z) * 10.0;
  q.y = q.y + sin(time * 2.0 + inc.x * 0.5 + inc.z * 0.5) * 100.0;
  q.y += 200;
  vec2 cap = sdCapsule(q, vec3(0, -FAR, 0), vec3(0, 0, 0), 25, 0.0);
  float ny = abs((q.y - 200) / 300.0);
  cap.y = ny;//fract(ny + random(vec2(inc.xz)));
  vec2 scene = cap;
...
vec4 render(vec3 ro, vec3 rd, Time time){
  ...
  if(d.x > 0.0){
    ...
    vec3 mat =texture2D(ramp, vec2(index, 0.5)).rgb;
    ...
    vec3 lightsPos[3] = vec3[3](
    	vec3(cos(PI * 0.5), 0.8, sin(PI * 0.05)),
    	vec3(cos(PI * 0.95), 0.8, sin(PI * 0.95)),
    	vec3(0.5, 0.5, 0.5)
    	);
    vec3 lightsColors[3] = vec3[3](
    	vec3(0.302, 0.3686, 1.0) * 0.85,//0.25,
    ...
    col += mat * lightColor + (SScol * subsurface) * intensity;// + specColor;//Lambert estimation + BlinnPhong 
    // col += mat;//unlit
